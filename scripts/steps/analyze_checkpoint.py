...
variance[valid_bins] = (agg_sum_coh_sq[valid_bins] / agg_count[valid_bins]) - mean_coh[
    valid_bins
] ** 2
...
if len(distances_fit) >= 3:
    try:
        ...
        print(f"   Correlation length (λ): {lambda_km:.1f} ± {perr[1]:.1f} km")
        ...
        ss_tot = np.sum(
            weights_fit
            * (coherences_fit - np.average(coherences_fit, weights=weights_fit)) ** 2
        )
        ...
        print(f"\n📊 DETAILED STATISTICS:")
        print(
            f"   Mean correlation:     {np.average(coherences_fit, weights=weights_fit):.4f}"
        )
        print(
            f"   Max correlation:      {coherences_fit.max():.4f} at {distances_fit[coherences_fit.argmax()]:.0f} km"
        )
        print(
            f"   Min correlation:      {coherences_fit.min():.4f} at {distances_fit[coherences_fit.argmin()]:.0f} km"
        )
        print(
            f"   Correlation range:    {coherences_fit.max() - coherences_fit.min():.4f}"
        )
        ...
        print(
            f"   Relative error:       {(perr[0] / amplitude) * 100:.1f}% (amplitude)"
        )
        ...
        for i, (low, high) in enumerate(zip(edges[:-1], edges[1:])):
            if agg_count[i] > 0:
                print(
                    f"   Bin {i:2d}: {low:7.0f}-{high:7.0f} km | {agg_count[i]:8,} pairs | μ={mean_coh[i]:7.4f} σ={std_coh[i]:7.4f}"
                )
        ...
        ax1.plot(
            r_smooth,
            correlation_model(r_smooth, *popt),
            "r-",
            linewidth=2,
        ...
        ax1.set_xscale("log")
        ax1.set_title(
            f"Preliminary TEP Analysis ({successful_files} files)",
            fontsize=14,
            fontweight="bold",
        )
        ...
        ax2.axhline(
            ...
            color="r",
            linestyle="--",
            label=f"Min threshold ({min_bin_count})",
        )
        ...
        print(f"   Expected λ range:     1,000 - 10,000 km (correlation length scale)")
        print(f"   Measured λ:           {lambda_km:.0f} ± {perr[1]:.0f} km")
        print(
            f"   Within theory range:  {'✅ YES' if 1000 <= lambda_km <= 10000 else '❌ NO'}"
        )
        ...
        print(
            f"   Max residual:         {max_residual:.6f} at {max_residual_dist:.0f} km"
        )
        ...
        for p in percentiles:
            ...
            print(
                f"   {p:2d}th percentile:     {dist_p:7.0f} km → correlation = {corr_p:.4f}"
            )
        ...
        print(
            f"   Expected final pairs: ~{int(total_pairs_kept * 9221 / successful_files):,}"
        )
        ...
        results = {
            ...
            "bin_statistics": {
                "mean_correlation": float(
                    np.average(coherences_fit, weights=weights_fit)
                ),
