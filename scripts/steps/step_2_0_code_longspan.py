...
Theoretical Background:
    ...
    with characteristic lengths λ determined by spatial variations in the
    temporal field (Temporal Topology) in the terrestrial environment.
...
Environment Variables (v0.17 defaults for published methodology):

  CORE ANALYSIS:
  - TEP_USE_PHASE_BAND: Use band-limited phase analysis (default: 1, phase-band method)
...
NAMESPACE = (
    os.getenv("TEP_LOG_NAMESPACE")
    or os.getenv("TEP_OUTPUT_NAMESPACE")
    or "code_longspan"
)
...
ROTATION_SIGNATURE_GRADIENT_STRENGTH = TEPConfig.get_float(
    "TEP_ROTATION_SIGNATURE_GRADIENT_STRENGTH"
)
ROTATION_SIGNATURE_LONGITUDE_CORR = TEPConfig.get_float(
    "TEP_ROTATION_SIGNATURE_LONGITUDE_CORR"
)
...
ANISOTROPY_CV_ISOTROPIC_THRESHOLD = TEPConfig.get_float(
    "TEP_ANISOTROPY_CV_ISOTROPIC_THRESHOLD"
)
ANISOTROPY_CV_CHAOTIC_THRESHOLD = TEPConfig.get_float(
    "TEP_ANISOTROPY_CV_CHAOTIC_THRESHOLD"
)
...
def print_performance_stats(
    start_time, files_processed, pairs_processed, memory_mb=None
):
    ...
    stats = (
        f"Performance: {files_per_sec:.1f} files/sec, {pairs_per_sec:,.0f} pairs/sec"
    )
    if memory_mb:
...
def atomic_save_checkpoint(
    checkpoint_file: Path, data: dict, max_retries: int = 3
) -> bool:
    """
    ...
    try:
        ...
        if c_range > 0:
            ...
            p0_robust = [
                ...
                p0[1],
                c_bs.min(),
            ]  # Keep lambda from main fit, update amplitude/offset
        else:
        ...
        try:
            ...
            p0_fallback = [
                max(0.01, c_range),
                3000,
                c_bs.min() if len(c_bs) > 0 else -0.02,
            ]
...
def load_station_coordinates():
    ...
    coord_file = (
        ROOT / f"data/coordinates/{NAMESPACE}/step_1_1_station_coords_global.csv"
    )

    # Use proper error handling instead of bare exceptions
    try:
        validate_file_exists(
            coord_file, "Station coordinates file (Ensure Step 1.1 is complete)"
        )
        coords_df = safe_csv_read(coord_file)
        step_logger.success(
            f"Loaded coordinates: {len(coords_df)} stations from {coord_file.name}"
        )
        return coords_df
...
def correlation_model(r, amplitude, lambda_km, offset):
    """
    TEP BAND CORRELATION MODEL: Exponential decay with correlation length
    =====================================================================
    ...
    - λ: Correlation length (1000-10000 km for viable TEP parameters)
    ...
    (Temporal Topology), where λ is the characteristic correlation length of
    field variation in the terrestrial environment. In dense regions, Temporal
    Shear (∇φ) is suppressed, yielding the observed decay of correlations with
    baseline distance.
    """
...
def power_law_model(r, amplitude, alpha, offset):
    ...
    return (
        amplitude * np.power(r + 1e-10, -alpha) + offset
    )  # Small offset (0.1mm) to avoid r=0
...
def power_law_with_cutoff_model(r, amplitude, alpha, cutoff_km, offset):
    ...
    return (
        amplitude * np.power(r + 1e-9, -alpha) * np.exp(-r / cutoff_km) + offset
    )  # Small offset (1nm) to avoid r=0
...
def matern_general_model(r, amplitude, length_scale, offset, nu):
    ...
    if nu == 0.5:  # Exponential
        ...
        return (
            amplitude
            * (1 + sqrt5_r_over_l + (5 / 3) * (r / length_scale) ** 2)
            * np.exp(-sqrt5_r_over_l)
            + offset
        )
    else:
        raise ValueError(
            f"Unsupported Matérn ν={nu}; only ν in {0.5, 1.5, 2.5} are implemented"
        )
...
def temporal_propagation_analysis(pair_data_with_coords, enable_temporal=True):
    ...
    try:
        ...
        for region, data in continental_regions.items():
            ...
            if len(pairs) >= 50:  # Minimum for reliable statistics
                ...
                correlation_strength = float(
                    np.sum(correlations > 0.1) / len(correlations)
                )  # Fraction with strong correlation

                # Distance-weighted correlation (closer pairs have more weight)
                weights = 1.0 / (
                    distances + 100.0
                )  # Add 100km offset to avoid division by zero
                ...
                regional_time_signatures[region] = {
                    ...
                    "longitude_center": float(
                        np.mean(
                            [p["station1_coords"][1] for p in pairs]
                            + [p["station2_coords"][1] for p in pairs]
                        )
                    ),
                    "latitude_center": float(
                        np.mean(
                            [p["station1_coords"][0] for p in pairs]
                            + [p["station2_coords"][0] for p in pairs]
                        )
                    ),
                }
        ...
        if len(regional_time_signatures) >= 3:
            ...
            available_regions = [
                r for r in rotation_sequence if r in regional_time_signatures
            ]
            ...
            if len(available_regions) >= 3:
                ...
                longitude_correlation = (
                    float(np.corrcoef(rotation_longitudes, rotation_correlations)[0, 1])
                    if len(rotation_correlations) > 1
                    else 0.0
                )

                # Earth rotation signature assessment (using centralized constants)
                has_rotation_signature = (
                    gradient_strength > ROTATION_SIGNATURE_GRADIENT_STRENGTH
                    and abs(longitude_correlation) > ROTATION_SIGNATURE_LONGITUDE_CORR
                )
                ...
                rotation_propagation = {
                    ...
                    "interpretation": "Earth rotation propagation signature detected"
                    if has_rotation_signature
                    else "No clear rotation propagation pattern",
                    "tep_assessment": "Strong evidence for Earth motion through structured field"
                    if has_rotation_signature
                    else "Spatial correlations without clear temporal propagation",
                }
...
def analyze_earth_motion_patterns(sector_stats):
    ...
    if len(ew_lambdas) >= 1 and len(ns_lambdas) >= 1:
        ...
        rotation_analysis = {
            ...
            "rotation_aligned": bool(
                abs(rotation_ratio - 1.0) > 0.2
            ),  # >20% difference
            "interpretation": f"E-W/N-S ratio = {rotation_ratio:.2f} "
            + (
                "(rotation-aligned anisotropy)"
                if abs(rotation_ratio - 1.0) > 0.2
                else "(minimal rotation effect)"
            ),
        }
    ...
    dipole_analysis = {
        ...
        "dipole_ratio": float(max_lambda / min_lambda)
        if min_lambda > 0
        else float("inf"),
    ...
    assessment = {
        ...
        "earth_motion_consistency": "Strong"
        if (
            rotation_analysis.get("rotation_aligned", False)
            and dipole_analysis["dipole_strength"] > DIPOLE_STRENGTH_THRESHOLD
        )
        else "Moderate",
    }
...
def _subsample_to_match_distribution(
    sector_distances, reference_distances, max_samples=5000
):
    """
    ...
    sector_hist, sector_bins = np.histogram(
        sector_distances, bins=ref_bins, density=True
    )
    ...
    for i in range(len(ref_bins) - 1):
        ...
        bin_mask = (sector_distances >= ref_bins[i]) & (
            sector_distances < ref_bins[i + 1]
        )
        ...
        if len(bin_indices) > 0:
            ...
            if n_sample > 0:
                ...
                sampled_indices = np.random.choice(
                    bin_indices, size=n_sample, replace=False
                )
...
def directional_anisotropy_test(pair_data_with_coords, enable_anisotropy=True):
    ...
    try:
        ...
        sector_data = {
            name: {"distances": [], "coherences": []} for name in sector_names
        }
        ...
        global_dist_hist, global_dist_bins = np.histogram(
            all_distances, bins=20, density=True
        )
        ...
        for sector, data in sector_data.items():
            ...
            sector_dist_hist, sector_dist_bins = np.histogram(
                sector_distances, bins=20, density=True
            )
            ...
            for i, dist in enumerate(sector_distances):
                ...
                if sector_dist_hist[sector_bin_idx] > 0:
                    weight = (
                        global_dist_hist[global_bin_idx]
                        / sector_dist_hist[sector_bin_idx]
                    )
                    weights[i] = weight
            ...
            matched_indices = _subsample_to_match_distribution(
                sector_distances,
                all_distances,
                max_samples=min(5000, len(sector_distances)),
            )
        ...
        for sector, matched_data in sector_data_matched.items():
            ...
            for i in range(len(dist_bins) - 1):
                mask = (distances_arr >= dist_bins[i]) & (
                    distances_arr < dist_bins[i + 1]
                )
            ...
            if len(bin_corrs) >= 5:  # Need enough bins for fitting
                try:
                    ...
                    for i in range(len(dist_bins) - 1):
                        mask = (distances_weighted >= dist_bins[i]) & (
                            distances_weighted < dist_bins[i + 1]
                        )
                        if np.sum(mask) >= 10:
                            # Weighted mean
                            weighted_coherence = np.average(
                                coherences_weighted[mask], weights=weights[mask]
                            )
                            weighted_distance = np.average(
                                distances_weighted[mask], weights=weights[mask]
                            )
                    ...
                    if len(bin_corrs_weighted) >= 5:
                        ...
                        popt_weighted, _ = curve_fit(
                            ...
                            p0=p0,
                            bounds=adaptive_bounds,
                            sigma=1 / np.sqrt(bin_weights_weighted),
                            maxfev=5000,
                        )
        ...
        if len(sector_stats) >= 4:  # Need reasonable directional coverage
            ...
            is_moderate_anisotropy = (
                ANISOTROPY_CV_MODERATE_LOWER < lambda_cv < ANISOTROPY_CV_MODERATE_UPPER
            )
            ...
            if is_too_isotropic:
                ...
                interpretation = (
                    "Moderate anisotropy (TEP-consistent - Earth moving through field)"
                )
            ...
            return {
                ...
                "anisotropy_category": "moderate"
                if is_moderate_anisotropy
                else "extreme"
                if is_too_anisotropic
                else "minimal",
                "n_sectors": len(sector_stats),
                "interpretation": interpretation,
                "tep_assessment": "Earth-motion-consistent anisotropy supports TEP"
                if is_moderate_anisotropy
                else "Investigate alignment with Earth motion vectors",
                ...
                "guardrail_summary": {
                    ...
                    "sectors_with_valid_matching": sum(
                        1
                        for r in distance_matching_results.values()
                        if r["distribution_matched"]
                    ),
                    "matching_methods": ["subsampling", "weighting"],
                    "validation_passed": all(
                        r["distribution_matched"]
                        for r in distance_matching_results.values()
                    )
                    if distance_matching_results
                    else False,
                },
            }
...
def jackknife_analysis(
    distances, coherences, weights, station_pairs_info=None, enable_jackknife=True
):
    """
    ...
    for i in range(n_samples):
        ...
        try:
            ...
            # Adaptive bounds based on data characteristics
            adaptive_bounds = TEPConfig.get_adaptive_lambda_bounds(
                distances[keep_indices]
            )

            popt, _ = curve_fit(
                correlation_model,
                distances[keep_indices],
                coherences[keep_indices],
                p0=p0,
                sigma=sigma,
                bounds=adaptive_bounds,
                maxfev=5000,
...
def run_leave_one_out_analysis(pair_level_df, analysis_type="loso"):
    ...
    if analysis_type == "loso":
        ...
        unique_items = pd.unique(
            pair_level_df[["station_i", "station_j"]].values.ravel("K")
        )
        item_column_i, item_column_j = "station_i", "station_j"
        step_logger.info(
            f"Starting LOSO analysis for {len(unique_items)} unique stations."
        )
    else:  # lodo
        unique_items = pair_level_df["date"].unique()
        item_column_i, item_column_j = (
            "date",
            "date",
        )  # Use the same column for filtering
    ...
    for item_to_exclude in unique_items:
        ...
        binned = (
            subset_df.groupby("dist_bin", observed=True)
            .agg(
...
def fit_model_with_aic_bic(
    distances, coherences, weights, model_func, p0, bounds, name
):
    ...
    try:
        ...
        for station1, station2 in itertools.combinations(stations, 2):
            ...
            try:
                dt_ns = np.median(
                    np.diff(
                        common_times.values.astype("datetime64[ns]").astype("int64")
                    )
                )
            ...
            plateau_value, plateau_phase = compute_cross_power_plateau(
                ...
                series2_common,
                fs=fs_hz,
                use_real_coherency=use_real_coherency,
                f1=f1,
                f2=f2,
            )
...
def compute_cross_power_plateau(
    series1: np.ndarray,
    series2: np.ndarray,
    fs: float,
    use_real_coherency: bool = False,
    f1: float = 0.001,
    f2: float = 0.01,
) -> Tuple[float, float]:
    """
    ...
    series1_detrended = series1 - np.polyval(
        np.polyfit(time_indices, series1, 1), time_indices
    )
    series2_detrended = series2 - np.polyval(
        np.polyfit(time_indices, series2, 1), time_indices
    )
    ...
    if use_real_coherency:
        ...
        # STEP 3: Band-limited phase averaging (published method)
        # ============================================================
        ...
        use_phase_band = (
            os.getenv("TEP_USE_PHASE_BAND", "1") == "1"
        )  # Default to phase-band method
        if use_phase_band:
            ...
            band_csd = cross_psd[
                band_mask
            ]  # Complex cross-spectral density in TEP band
...
def calculate_baseline_distance(
    station1: str, station2: str, coords_df: pd.DataFrame
) -> Optional[float]:
    """
...
def build_distance_cache(coords_df: pd.DataFrame) -> Dict[Tuple[str, str], float]:
    ...
    if cache_file.exists():
        try:
            ...
            step_logger.success(
                f"Distance cache loaded: {len(distance_cache):,} cached distances"
            )
    ...
    step_logger.success(
        f"Distance cache complete: {len(distance_cache):,} cached distances"
    )
    return distance_cache
        ...
        if clk_file.suffix == ".Z":
            ...
            if not temp_gz_file.exists():
                ...
                if result.returncode != 0:
                    return {
                        "file": clk_file.name,
                        "error": f"Failed to convert {clk_file.name}: {result.stderr}",
                    }
        ...
        if ("dist_km" not in df_file.columns) or (df_file["dist_km"].isna().all()):
            if distance_cache:
                ...
                def get_cached_distance(row):
                    code1 = (
                        row["station_i"][:4]
                        if len(row["station_i"]) > 4
                        else row["station_i"]
                    )
                    code2 = (
                        row["station_j"][:4]
                        if len(row["station_j"]) > 4
                        else row["station_j"]
                    )
                    return distance_cache.get((code1, code2))

                df_file["dist_km"] = df_file[["station_i", "station_j"]].apply(
                    get_cached_distance, axis=1
                )
            else:
                # Fallback to direct calculation
                df_file["dist_km"] = df_file[["station_i", "station_j"]].apply(
                    lambda r: calculate_baseline_distance(
                        r["station_i"], r["station_j"], coords_df
                    ),
                    axis=1,
                )
...
def process_analysis_center(
    ...
    all_clk_files = sorted(
        list(clk_dir.glob("*.CLK.gz")) + list(clk_dir.glob("*.CLK.Z"))
    )
    ...
    if not clk_files:
        step_logger.warning(
            f"No {ac.upper()} .CLK.gz files found in the specified date range"
        )
    ...
    step_logger.info(
        f"File size range: {min(f.stat().st_size for f in clk_files) / 1024 / 1024:.1f} - {max(f.stat().st_size for f in clk_files) / 1024 / 1024:.1f} MB"
    )
    ...
    step_logger.success(
        f"Binning configuration: {num_bins} bins from 50 to {max_distance} km"
    )
    step_logger.info(f"Minimum {min_bin_count} pairs required per bin for fitting")
    step_logger.info(
        f"Distance bin edges: {edges[0]:.1f}, {edges[1]:.1f}, ..., {edges[-2]:.1f}, {edges[-1]:.1f} km"
    )

    # Get number of workers using centralized configuration
    num_workers = TEPConfig.get_worker_count()
    step_logger.success(
        f"Using {num_workers} parallel workers ({mp.cpu_count()} CPU cores available)"
    )
    ...
    max_anisotropy_samples = int(
        os.getenv("TEP_ANISOTROPY_SAMPLES", "10000")
    )  # Limit for memory
    ...
    write_pair_level = TEPConfig.get_bool(
        "TEP_WRITE_PAIR_LEVEL", True
    )  # Default TRUE for pipeline continuity
    max_memory_pairs = int(
        os.getenv("TEP_MAX_MEMORY_PAIRS", "100000")
    )  # Reduced from 500K to 100K

    # Log memory optimization settings and pipeline requirements
    if write_pair_level:
        step_logger.info(
            "Pair-level CSV writing enabled (will use ~15-20 GB disk space)"
        )
        step_logger.info(
            "Required for Steps 2.1 (Data Quality) and 2.2 (Geospatial/Temporal Analysis)"
        )
    else:
        step_logger.info("Pair-level CSV writing is disabled (memory-optimized mode)")
        step_logger.warning(
            "WARNING: Steps 2.1 and 2.2 will FAIL without pair-level files!"
        )
        step_logger.warning(
            "Set TEP_WRITE_PAIR_LEVEL=True in config to enable full pipeline"
        )
    ...
    if resume_enabled:
        ...
        if state:
            ...
            step_logger.info(
                f"Resumed from checkpoint: {len(processed_files)} files already processed"
            )
        else:
            processed_files = set()
    ...
    if not remaining_files:
        ...
        if distance_cache is None:
            ...
            step_logger.info(
                f"Using provided distance cache with {len(distance_cache):,} cached distances"
            )
        ...
        step_logger.process(
            f"Initializing persistent worker pool with {num_workers} workers..."
        )
        ...
        with ProcessPoolExecutor(
            ...
            for batch_start in range(0, len(remaining_files), batch_size):
                ...
                step_logger.process(
                    f"Processing batch {batch_start // batch_size + 1}: {len(batch_files)} files"
                )

                future_to_file = {
                    executor.submit(process_file_worker, f): f for f in batch_files
                }
                    ...
                    try:
                        ...
                        if result and "error" not in result:
                            ...
                            if write_pair_level and "pair_data_buffer" in result:
                                consolidated_pair_data.extend(
                                    result["pair_data_buffer"]
                                )

                                # Proactive memory management: flush to disk if approaching limit
                                if len(consolidated_pair_data) > max_memory_pairs:
                                    step_logger.info(
                                        f"Memory limit reached ({len(consolidated_pair_data):,} pairs), flushing to disk..."
                                    )
                                    # Write current buffer to temporary file
                                    temp_file = (
                                        checkpoint_dir
                                        / f"temp_pairs_{ac}_{len(consolidated_pair_data)}.csv"
                                    )
                            ...
                            step_logger.success(
                                f"{result['file']}: {result['pairs_processed']:,} pairs"
                            )
                ...
                if atomic_save_checkpoint(checkpoint_file, checkpoint_data):
                    step_logger.info(
                        f"Checkpoint saved: {len(processed_files)}/{len(clk_files)} files processed"
                    )
                else:
                    step_logger.warning(
                        "Failed to save checkpoint - continuing without checkpoint"
                    )
    ...
    step_logger.success(
        f"Total kept pairs: {total_pairs_kept:,} from {successful_files} files"
    )
    ...
    step_logger.debug(
        f"DEBUG: Number of bins for fitting (len(distances)): {len(distances)}"
    )
    ...
    try:
        ...
        models_to_fit = [
            ...
            },
            {
                ...
                "bounds": (
                    [1e-10, 0.1, 1000, -1],
                    [5, 10, min(15000, max(distances) * 0.8), 1],
                ),
            },
            {
            ...
            },
            {
                ...
                "func": lambda r, amp, l, off: matern_general_model(
                    r, amp, l, off, nu=2.5
                ),
                "name": "Matérn (ν=2.5)",
                "p0": [c_range, initial_lambda_guess, coherences.min()],
        ...
        for result in sorted(successful_models, key=lambda x: x["aic"]):
            ...
            step_logger.info(
                f"{result['name']:15s} | {result['aic']:8.2f} | {result['bic']:8.2f} | {result['r_squared']:6.3f} | {delta_aic:6.2f}"
            )
        ...
        exp_result = next(
            (r for r in model_results if r["name"] == "Exponential" and r["success"]),
            None,
        )
        ...
        if exp_result:
            ...
            exp_amplitude_err, exp_lambda_err, exp_offset_err = (
                amplitude_err,
                lambda_err,
                offset_err,
            )
        ...
        jackknife_lambdas = jackknife_analysis(
            distances, coherences, weights, enable_jackknife=enable_jackknife
        )
        ...
        if jackknife_lambdas:
            ...
            step_logger.info(
                f"Jackknife analysis: λ = {jackknife_mean:.1f} ± {jackknife_std:.1f} km ({len(jackknife_lambdas)} samples)"
            )
        else:
            jackknife_mean = jackknife_std = None

        step_logger.info(
            "Core correlation analysis complete. Run Step 2.1 for data quality validation, then Step 2.2 for geospatial temporal analysis."
        )
        ...
        results = {
            ...
            "tep_interpretation": {
                "tep_consistent": bool(
                    1000 < exp_lambda_km < 10000 and exp_r_squared > 0.3
                ),
                "correlation_length_assessment": "TEP-consistent"
                if 1000 < exp_lambda_km < 10000
                else "Outside TEP range",
                "signal_strength": "Strong"
                if exp_r_squared > 0.5
                else "Moderate"
                if exp_r_squared > 0.3
                else "Weak",
                "best_model_vs_exponential": f"Best model: {best_result['name']} (ΔAIC = {best_result['aic'] - exp_result['aic']:.2f})"
                if exp_result and "aic" in exp_result
                else "Exponential model failed",
            },
            "loso_analysis": None,
            "lodo_analysis": None,
        }
        ...
        step_logger.info(
            f"  Correlation Length (λ): {lambda_km:.1f} ± {lambda_err:.1f} km"
        )
        ...
        if best_result["name"] != "Exponential" and exp_result:
            ...
            step_logger.info(
                f"  Amplitude (A): {exp_amplitude:.6f} ± {exp_amplitude_err:.6f}"
            )
            step_logger.info(
                f"  Correlation Length (λ): {exp_lambda_km:.1f} ± {exp_lambda_err:.1f} km"
            )
        ...
        if results["tep_interpretation"]["tep_consistent"]:
            ...
            step_logger.info(
                f"  Exponential model: λ = {exp_lambda_km:.0f} km is in TEP range"
            )
            step_logger.info(
                f"  R² = {exp_r_squared:.3f} indicates {results['tep_interpretation']['signal_strength'].lower()} correlation structure"
            )
            step_logger.info(f"  Phase-coherent analysis supports TEP predictions")
            if best_result["name"] != "Exponential":
                step_logger.info(
                    f"  Note: {best_result['name']} model fits better (ΔAIC = {best_result['aic'] - exp_result['aic']:.2f})"
                )
        else:
        ...
        if best_result["name"] == "Exponential":
            ...
            coherences_pred = squared_exponential_model(
                distances, *best_result["params"]
            )
            ...
            coherences_pred = power_law_with_cutoff_model(
                distances, *best_result["params"]
            )
            ...
            coherences_pred = matern_general_model(
                distances, *best_result["params"], nu=2.5
            )
        else:
            coherences_pred = correlation_model(
                distances, *best_result["params"]
            ...
            with open(consolidated_csv, "w") as f:
                ...
                for temp_file in temp_files:
                    ...
                    try:
                        if temp_file.exists():
                            ...
                            step_logger.debug(
                                f"Temp file already cleaned up: {temp_file.name}"
                            )
                    except Exception as e:
                        step_logger.warning(
                            f"Warning: Could not remove temp file {temp_file.name}: {e}"
                        )
                ...
                for individual_file in individual_files:
                    ...
                    try:
                        ...
                        step_logger.debug(
                            f"Cleaned up individual file: {individual_file.name}"
                        )
                    except Exception as e:
                        step_logger.warning(
                            f"Warning: Could not remove individual file {individual_file.name}: {e}"
                        )

            if total_pairs_written > 0:
                step_logger.success(
                    f"Consolidated pair data saved: {consolidated_csv} ({total_pairs_written:,} pairs)"
                )
                if temp_files or individual_files:
                    step_logger.success(
                        f"Merged {len(temp_files)} temp files and {len(individual_files)} individual files into consolidated output"
                    )
            else:
        ...
        if bootstrap_iter > 0:
            step_logger.info(
                f"Running bootstrap ({bootstrap_iter} iterations) for CI with improved stability"
            )
            ...
            chunk_size = (
                min(100, bootstrap_iter // num_workers)
                if num_workers > 1
                else bootstrap_iter
            )
            ...
            for chunk_idx in range(total_chunks):
                ...
                # Process chunk
                with ProcessPoolExecutor(
                    max_workers=min(num_workers, end_idx - start_idx)
                ) as executor:
                    # Prepare arguments for this chunk
                    chunk_tasks = [
                        (distances, coherences, weights, p0_bootstrap, i)
                        for i in range(start_idx, end_idx)
                    ]

                    # Submit tasks and get futures
                    future_to_iter = {
                        executor.submit(fit_bootstrap_task, task): i
                    ...
                    step_logger.info(
                        f"Bootstrap chunk {chunk_idx + 1}/{total_chunks}: {chunk_results} successful fits"
                    )
            ...
            if bs_amp:
                ...
                step_logger.success(
                    f"Bootstrap completed: {len(bs_amp)}/{bootstrap_iter} successful ({success_rate:.1f}%)"
                )

                ci_low = 2.5
                ci_high = 97.5
                amp_ci = [
                    float(np.percentile(bs_amp, ci_low)),
                    float(np.percentile(bs_amp, ci_high)),
                ]
                lambda_ci = [
                    float(np.percentile(bs_lambda, ci_low)),
                    float(np.percentile(bs_lambda, ci_high)),
                ]
                offset_ci = [
                    float(np.percentile(bs_offset, ci_low)),
                    float(np.percentile(bs_offset, ci_high)),
                ]
...
def main():
    ...
    print_status(
        "Detecting TEP signatures through phase-coherent clock correlation analysis",
        "INFO",
    )
    print_status("=" * 80, "INFO")
    ...
    if use_real_coherency:
        ...
        print_status(
            "Note: Full implementation requires time series data access", "INFO"
        )
    else:
        print_status("", "INFO")
    ...
    if missing_files_any_center:
        print_status(
            "CRITICAL: Step 1.1 data acquisition not completed for requested centers. Please run Step 1.1 first.",
            "ERROR",
        )
        return False
    else:
        print_status(
            "Step 1.1 data acquisition verified for requested centers.", "SUCCESS"
        )
    ...
    for ac in centers:
        ...
        if result and "exponential_fit" in result and result["exponential_fit"]:
            ...
            print_status(
                f"{ac.upper()} processing failed or produced no valid exponential fit data.",
                "ERROR",
            )
            # Ensure the entry for this AC is still in results, but marked as failed or empty if needed by other steps
            results[ac] = {
                "error": True,
                "message": f"{ac.upper()} processing failed or produced no valid exponential fit data.",
            }
    ...
    if results:
        ...
        successful_results = {
            ...
            if not res.get("error") and "best_fit" in res
        }
        if successful_results:
            ...
            for ac, result in successful_results.items():
                ...
                print_status(
                    f"  Model: {best_fit['model_name']} (R²={best_fit['r_squared']:.3f})",
                    "INFO",
                )
                print_status(
                    f"  Correlation length (λ): {best_fit['lambda_km']:.1f} ± {best_fit['lambda_error']:.1f} km",
                    "INFO",
                )
                print_status(
                    f"  Amplitude (A): {best_fit['amplitude']:.3e} ± {best_fit['amplitude_error']:.3e}",
                    "INFO",
                )
                print_status(
                    f"  Offset (C₀): {best_fit['offset']:.3e} ± {best_fit['offset_error']:.3e}",
                    "INFO",
                )
                print_status(
                    f"  Exponential model λ: {exp_fit['lambda_km']:.1f} km (R²={exp_fit['r_squared']:.3f})",
                    "INFO",
                )
                print_status(
                    f"  TEP Interpretation: {tep['correlation_length_assessment']} (Signal: {tep['signal_strength']})",
                    "INFO",
                )
                print_status("", "INFO")
        else:
            print_status(
                "No successful analyses to report (all centers failed or had no data)",
                "WARNING",
            )
    ...
    global_summary_path = (
        ...
        / "outputs"
        / NAMESPACE
        / "step_2_0_correlation_analysis_summary.json"
    )
    ...
    print_status(
        "Exploratory run: provenance tracking skipped (isolated from main pipeline)",
        "INFO",
    )
